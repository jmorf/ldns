  ...
  {:else if records.length > 0}
    <div class="bg-surface border border-line rounded-xl divide-y divide-line">
      {#each records as record}
        <div class="p-4 flex items-start justify-between gap-3 hover:bg-surface-2/50 transition-colors">
          <div class="flex-1 min-w-0">
            <p class="font-mono text-sm text-fg break-all">{record.data}</p>
            <p class="text-[11px] text-fg-subtle mt-1">TTL: <span class="font-mono tabular-nums text-fg-muted">{record.ttl}s</span></p>
          </div>
