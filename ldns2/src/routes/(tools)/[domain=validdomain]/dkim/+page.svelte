  ...
  {:else if result}
    {#if result.found === 0}
      <div class="bg-gray-900 border border-gray-700 rounded-xl p-8 text-center">
        <p class="text-sm text-gray-400">No DKIM records found at any of the {result.probed} probed selectors.</p>
        <p class="text-[11px] text-gray-500 mt-2">DKIM keys live at <span class="font-mono">selector._domainkey.{domain.name}</span>. If this domain uses a non-standard selector we may not detect it.</p>
      </div>
    {:else}
        ...
        {#each result.selectors as sel}
          <div class="p-4">
            <div class="flex items-center justify-between mb-2">
              ...
              <span class="flex items-center gap-2 text-[10px] text-gray-400">
                ...
                {#if sel.policy === 'y'}<span class="px-1 py-0.5 bg-yellow-500/15 text-yellow-400 border border-yellow-500/30 rounded">testing</span>{/if}
              </span>
            </div>
            <p class="text-[11px] font-mono text-gray-400 break-all">{sel.raw}</p>
          </div>
        {/each}
      </div>
    {/if}
