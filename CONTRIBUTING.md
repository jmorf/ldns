...
- **Bugs and feature requests** → [open an issue](https://github.com/jmorf/ldns/issues/new/choose). For anything non-trivial, please open an issue *before* writing code to check the approach first, it's no fun to have a PR turned down after the work is done.
...
- **No em dashes, en dashes or double hyphens** in code, comments, docs or UI copy. Rewrite the
  sentence instead. Regular hyphens in compound words are fine.
- **First person singular in user-facing copy.** Use "LDNS" for what the software does and "I"
  for the maintainer, never "we".
