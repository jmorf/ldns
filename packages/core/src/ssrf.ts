...
export async function ensurePublicHost(domain: string): Promise<{ ok: true } | { ok: false; reason: string }> {
  if (!isPlausibleDomain(domain)) return { ok: false, reason: REJECTED };
  ...
  if (ips.length === 0) return { ok: false, reason: REJECTED };

  for (const ip of ips) {
    if (ip.includes(':')) {
      if (isPrivateIPv6(ip)) return { ok: false, reason: REJECTED };
